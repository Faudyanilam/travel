...
if (isset($_POST["submit"])) {
    ...
    $sql = "INSERT INTO mandiri (nama, nomor_telp, email, alamat, jumlah, tanggal_pemesanan, resort, durasi, tambahan, harga) 
            VALUES('$nama', '$nomor_telp', '$email', '$alamat', '$jumlah', '$tanggal_pemesanan', '$resort', '$durasi', '" . implode(", ", $tambahan) . "', '$harga')";
    $result = mysqli_query($koneksi, $sql);

    if ($result) {
        echo "<script>alert('Data berhasil ditambahkan');
        window.location='modifikasi_mandiri.php'</script>";
    } else {
        echo "<script>alert('Data gagal ditambahkan!');
        window.location='mandiri.php'</script>";
    }
}
?>
  ...
  <section class="section-padding bg-gray">
    <div class="container">
      ...
      <form class="search-form" method="post">
            ...
            <div class="form-group text-left">
              <label>Resort <span style="color: red;">*</span></label>
                <select class="form-control" name="resort" required>
                  <option value="" disabled selected>Pilih Resort</option>
                  <option value="Panbil Nature Reserve">Panbil Nature Reserve</option>
                  <option value="Nongsa Point Marina">Nongsa Point Marina</option>
                  <option value="Nuvasa">Nuvasa</option>
                </select>
            </div>
            <div class="form-group text-left">
              <label>Durasi Liburan <span style="color: red;">*</span></label>
                <div class="input-group">
                  <input type="number" class="form-control" placeholder="Berapa hari anda akan berlibur?" name="durasi" required>
                </div>
            </div>
            <div class="form-group text-left">
                <label>Pilih Tambahan: <span style="color: red;">*</span></label><br/>
                <input type="checkbox" name="tambahan[]" value="Penginapan"> Penginapan (Rp. 1.000.000)<br/>
                <input type="checkbox" name="tambahan[]" value="Transportasi"> Transportasi (Rp. 1.200.000)<br/>
                <input type="checkbox" name="tambahan[]" value="Makanan"> Makanan (Rp. 500.000)<br/>
            </div>
